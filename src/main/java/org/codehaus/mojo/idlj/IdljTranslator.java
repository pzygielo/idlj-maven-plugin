 ...
 * @version $Id$
 */
public class IdljTranslator extends AbstractTranslator implements
		CompilerTranslator {
	 ...
	 * @param source
	 *            the source tag available in the configuration tree of the
	 *            maven plugin
	 ...
	 *      Source)
	 */
	public void invokeCompiler(String sourceDirectory, List includeDirs,
		...
		args.add("-td");
		args.add(toRelativeAndFixSeparator(new File(System
				.getProperty("user.dir")), new File(targetDirectory), false));

		if (source.getPackagePrefix() != null) {
			throw new MojoExecutionException(
					"idlj compiler does not support packagePrefix");
		}

		if (source.getPackagePrefixes() != null) {
			for (Iterator prefixes = source.getPackagePrefixes().iterator(); prefixes
					.hasNext();) {
		...
		if (source.getDefines() != null) {
			for (Iterator defs = source.getDefines().iterator(); defs.hasNext();) {
				...
				if (define.getValue() != null) {
					throw new MojoExecutionException(
							"idlj compiler unable to define symbol values");
				}
				args.add("-d");
		...
		if (source.getAdditionalArguments() != null) {
			for (Iterator it = source.getAdditionalArguments().iterator(); it
					.hasNext();) {
				args.add(it.next());
			}
		}
		...
		try {
			...
			try {
				...
				if (System.getProperty("java.vm.name").indexOf("HotSpot") != -1) {
					urlLoader
							.loadClass("com.sun.tools.corba.se.idl.som.cff.FileLocator");
				}
				idljCompiler = urlLoader.loadClass(getIDLCompilerClass());
			} catch (Exception notUsed) {
				throw new MojoExecutionException(" IDL compiler not available",
						e);
			}
		}
		return idljCompiler;
	...
	private void invokeCompiler(Class compilerClass, List args)
		...
		try {
			compilerMainMethod = compilerClass.getMethod("main",
					new Class[] { String[].class });
		} catch (NoSuchMethodException e1) {
			throw new MojoExecutionException(
					"Error: Compiler had no main method");
		}
		...
		if (isFailOnError()
				&& (exitCode != 0 || err.toString().indexOf("Invalid argument") != -1)) {
